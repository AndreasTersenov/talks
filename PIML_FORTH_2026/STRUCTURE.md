...
| 15 | Plug-and-play is within 1 % of the end-to-end network, with the smallest calibrated error bars, **redrawn** for this deck: the summary chart as an inline SVG with the methods named as the earlier frames named them (ℓ2 prior, sparse solver, end to end, plug-and-play and its residual variant), circles before and diamonds after the conformal step, a reading key beside it; the σ maps as a card on the first click, the trained-once line on the second | 24, redrawn |
