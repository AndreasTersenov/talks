#!/usr/bin/env python3
"""The three-panel CQR schematic at talk size, for PIML_FORTH_2026 frame 14 (the
backup-card version is make-cqr-schematic.py; same numbers, bigger type).
...
Writes assets/diagrams/cqr_schematic_talk.png.
"""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
fig, ax = plt.subplots(1, 3, figsize=(16.4, 4.4))
...
def strip(a):
    ...
    a.tick_params(colors=MUTED, labelsize=16, length=3)
...
def bars(a, half_i, title):
    ...
    a.vlines(x, centre - half_i, centre + half_i, color=ACCENT, lw=5.2,
             alpha=0.28, zorder=1)
    a.hlines(centre - half_i, x - 0.26, x + 0.26, color=ACCENT, lw=1.5, zorder=2)
    a.hlines(centre + half_i, x - 0.26, x + 0.26, color=ACCENT, lw=1.5, zorder=2)
    a.scatter(x[inside], truth[inside], s=46, color=INK, zorder=3)
    a.scatter(x[~inside], truth[~inside], s=110, color=WARN, zorder=4,
              marker="X", linewidths=0)
    a.set_xlim(-0.9, N - 0.1)
    a.set_ylim(-3.6, 3.6)
    a.set_xticks([])
    a.set_yticks([-2, 0, 2])
    a.set_title(title, color=INK, fontsize=20, pad=10)
...
ax[0].set_ylabel(r"$\kappa$ at one pixel", color=MUTED, fontsize=18)

srt = np.sort(score)
ax[1].scatter(np.arange(N), srt, s=58, color=INK, zorder=3)
...
ax[1].text(N + 0.25, q, "$q$", color=WARN, fontsize=24, va="center", ha="left")
...
ax[1].set_xticks([])
ax[1].set_title("2.  score each one, take the quantile", color=INK, fontsize=20, pad=10)
caption(ax[1], r"$s=\max(\hat\kappa^- - \kappa,\; \kappa - \hat\kappa^+)$, sorted")
ax[1].set_ylabel("conformity score", color=MUTED, fontsize=18)
...
ax[0].text(0.0, 1.17, "schematic", transform=ax[0].transAxes, ha="left",
           va="bottom", color=MUTED, fontsize=16, style="italic")

fig.subplots_adjust(left=0.058, right=0.985, top=0.82, bottom=0.19, wspace=0.22)
fig.savefig("assets/diagrams/cqr_schematic_talk.png", dpi=200, transparent=True)
print("wrote assets/diagrams/cqr_schematic_talk.png  (%d -> %d miscovered of %d, q = %+.2f)"
      % (m0, m1, N, q))
