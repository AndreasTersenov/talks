...
Adapted for the slide: deck palette, heavier labels, and the stage captions
carried by the brackets instead of by prose.

    /usr/local/bin/python3 tools/make-sbi-pipeline.py

Writes assets/diagrams/sbi_pipeline_general.png.
...
def blob(cx, cy, sx, sy, seed, alphas=(0.18, 0.34, 0.85), scales=(1.6, 1.1, 0.6)):
    t = np.linspace(0, 2 * np.pi, 400)
    r = 1 + 0.28 * np.sin(3 * t + seed) + 0.15 * np.cos(2 * t - seed)
    for sc, a in zip(scales, alphas):
...
keep = (px > 4) & (px < 22) & (py > 7.5) & (py < 19)   # no strays under the box
...
# ============================================================== 2. learning
ax.text(37, 40.5, r"base  $\mathcal{N}$", ha="center", fontsize=12.5, color=INK)
...
for cx, k in cols:
    ...
    ax.scatter([cx] * k, ys, s=70, facecolor=PAPER, edgecolor=ACCENT, lw=1.3,
               zorder=3)
    prev = (cx, ys)
ax.text(58.7, 40.5, r"conditional flow   $q_\phi(\theta \mid x)$",
        ha="center", fontsize=12.5, color=ACCENT)

# the pairs travelling from stage 1 into the network
...
ax.text(40, 15.4, r"the pairs  $(\theta,\, x)$", ha="center", fontsize=12,
        color=INK)
ax.text(40, 10.6, "trained by maximum likelihood", ha="center", fontsize=10.5,
        color=MUTED)
...
ax.text(84, 14.5, r"posterior estimate", ha="center", fontsize=12, color=INK)
...
STAGES = [(3.5, 22.5, "1.  Simulation", "draw $\\theta$, simulate,\nkeep the $(\\theta,\\, x)$ pairs"),
          (25.5, 69.5, "2.  Learning",  "fit the conditional flow\non those pairs"),
          (72.5, 96.5, "3.  Inference", "evaluate at $x_{\\rm obs}$;\ncheap to repeat")]
for x0, x1, name, sub in STAGES:
    ax.plot([x0, x0, x1, x1], [-0.8, -2.4, -2.4, -0.8], color=MUTED, lw=1.1)
    ax.text((x0 + x1) / 2, -4.6, name, ha="center", va="top", fontsize=13,
            color=INK)
    ax.text((x0 + x1) / 2, -6.4, sub, ha="center", va="top", fontsize=10.5,
            color=MUTED, linespacing=1.4)

fig.subplots_adjust(0, 0, 1, 1)
fig.savefig("assets/diagrams/sbi_pipeline_general.png", facecolor=PAPER,
            bbox_inches="tight", pad_inches=0.06)
print("wrote assets/diagrams/sbi_pipeline_general.png")
